...
<head>
    ...
    <style>
        ...
        /* 🚀 PERBAIKAN TOMBOL (Gradien Lebih Kontras dan Jelas Bentuknya) */
        .gradient-button {
...
<body class="font-['Inter'] antialiased">
    ...
    <div class="min-h-screen flex items-center justify-center p-4 premium-bg"> 
        ...
        {{-- Kartu Formulir Login: Glassmorphism Mewah --}}
        <div class="w-full max-w-md p-8 sm:p-10 rounded-2xl text-white glass-card">
            ...
            <div class="mb-8 text-center">
                ...
                <p class="text-sm text-white/80 mt-1">Sistem manajemen keuangan yang terpercaya</p>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div class="block mt-4 flex items-center justify-between text-white">
                    <label for="remember_me" class="inline-flex items-center select-none">
                        <input id="remember_me" type="checkbox" class="rounded border-white/50 text-white bg-white/20 shadow-sm focus:ring-white w-4 h-4" name="remember">
                        <span class="ml-2 text-sm text-white/90">Ingat Saya</span>
                    </label>

                    @if (Route::has('password.request'))
                        <a class="text-sm font-semibold text-white/90 hover:text-white transition duration-150 ease-in-out underline" href="{{ route('password.request') }}">
                            Lupa Kata Sandi?
                        </a>
                    @endif
                </div>

                {{-- Tombol Login: Gradien Biru Halus & Jelas --}}
            ...
            {{-- Tautan Daftar (Diperjelas) --}}
            <div class="mt-6 text-center pt-4 border-t border-white/30">
                <p class="text-sm text-white/80">
                    Belum punya akun? 
                    <a href="{{ route('register') }}" class="font-bold text-white hover:text-gray-100 transition duration-150 ease-in-out underline">
                        Daftar Akun Baru
                    </a>
                </p>
            </div>
